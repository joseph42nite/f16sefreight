...
| 282 | ❓ **Doc vs code: a card already sent to the airline is still painted red** | PRD §5.5 says green once the document is sent; `JobBoard.urgency()` looks only at the clearance date, and says it follows the OLI bands on purpose — `OperatorLoadService` counts Sent to Airline and Airline Confirmed jobs as active, weighted by the same date. The demo's In Transit cards cleared yesterday, so they show red. Changing the board alone would make it disagree with the load index. **Your call:** should sent jobs stop counting as urgent in both? |
