...
| 280 | 🟢 **Kanban cards link to their mail and message log; In Transit shows where the shipment is** | User, 2026-09-15. Bottom right of a card: ✉ opens `/inbox?thread=` on the job's conversation (the job's own thread, else its enquiry's; no icon when there is no mail); on In Transit, ⌸ opens `/message-log?awb=` with the AWB filled in and searched. The progress bar reads the airline's **Cargo Status** messages for the AWB — the rows the Message Log shows — into PRD §5.5's seven steps: Accepted (FOH RCS DPU FIW) · Manifested (PRE MAN TRM) · Departed (DEP) · Arrived (ARR RCF AWR) · Customs cleared (CCD) · Out for delivery (NFD AWD FOW) · Delivered (DLV DDL). The furthest step reached is shown; DIS adds a discrepancy warning. ❓ **Which code is which step is our reading of the code descriptions** (core/config/cargoMilestones.js) — correct it there. ⚠️ On a multi-leg flight an ARR at the transfer airport already reads "Arrived": the status message does not say where. ⚠️ Doc vs request: PRD/ui guide put ✉ on all four columns and the request named three — ✉ is on all four, In Transit included. Not built from the same PRD table: the card-body click behaviours and the AWB tracking drawer. Tests: JobBoardLinksTest, cargoMilestones.spec.js |
