...
class IMPConversionController extends Controller
{
    ...
    public function HouseWayBillConversion($hawb_no = '12345678')
    {
        ...
        $main_data = "FZB/6<br>";
        //2
        $main_data .= "HCD/" . $house_data['awb_code'] . '-' . $house_data['awb_no'] . "/" . $house_data['id'] . "/" . substr($house_data['departure_airport'], 0, 3) . substr($house_data['destination_airport'], 0, 3) . '/' . "T" . $consignment_data['pieces'] . "K" . $consignment_data['gross_weight'] . "<br>";
        //3
        $main_data .= "FLT/" . $house_data['by'] . $house_data['flight'] . "/" . substr($house_data['date'], 0, 2);
        if ($house_data['by_2'])
            $main_data .= "/" . $house_data['by_2'] . $house_data['flight_2'] . "/" . substr($house_data['date_2'], 0, 2);
        if ($house_data['by_3'])
            $main_data .= "/" . $house_data['by_3'] . $house_data['flight_3'] . "/" . substr($house_data['date_3'], 0, 2);
        $main_data .= "<br>";
        //4
        $main_data .= "RTG/" . substr($house_data['to'], 0, 3) . $house_data['by'];
        if ($house_data['to_2'])
            $main_data .= "/" . substr($house_data['to_2'], 0, 3) . $house_data['by_2'];
        if ($house_data['to_3'])
            $main_data .= "/" . substr($house_data['to_3'], 0, 3) . $house_data['by_3'];
        $main_data .= "<br>";
        //shiper address
        //5
        $main_data .= "SHP<br>";
        //6
        $main_data .= "NAM/" . $house_address['ship_name'] . "<br>";
        //7
        $main_data .= "ADR/" . $house_address['ship_address'] . "/" . $house_address['ship_address_line_2'] . "<br>";
        //8
        $main_data .= "LOC/" . $house_address['ship_city'] . "/" . substr($house_address['ship_state'], 0, 2) . "/" . substr($house_address['ship_country'], 0, 2) . "/" . $house_address['ship_post_code'] . "/" . "TE/" . $house_address['ship_telex'];
        $main_data .= "<br>";
        //consignee address
        //9
        $main_data .= "CNE<br>";
        //10
        $main_data .= "NAM/" . $house_address['cons_name'] . "<br>";
        //11
        $main_data .= "ADR/" . $house_address['cons_address'] . "/" . $house_address['cons_address_line_2'] . "<br>";
        //12
        $main_data .= "LOC/" . $house_address['cons_city'] . "/" . substr($house_address['cons_state'], 0, 2) . "/" . substr($house_address['cons_country'], 0, 2) . "/" . $house_address['cons_post_code'] . "/" . "TE/" . $house_address['cons_telex'];
        $main_data .= "<br>";
        //18//agent details
        $main_data .= 'AGT//' . $agent_details['iata_agent_code'] . '/' . $agent_details['iata_agent_cass'] . "<br>";
        //19
        ...
        if (!empty($house_data['special_service_request']))
            $main_data .= "SSR/" . $house_data['special_service_request'] . "<br>";
        //also notify
        if (!empty($house_address['also_name'])) {
            $main_data .= "NFY<br>";
            $main_data .= "NAM/" . $house_address['also_name'] . "<br>";
            $main_data .= "ADR/" . $house_address['also_address'] . "/" . $house_address['also_address_line_2'] . "<br>";
            $main_data .= "LOC/" . $house_address['also_city'] . "/" . substr($house_address['also_state'], 0, 2) . "/" . substr($house_address['also_country'], 0, 2) . "/" . $house_address['also_post_code'] . "/" . "TE/" . $house_address['also_telex'];
            $main_data .= "<br>";
        }
        //21
        $main_data .= "CVD/" . $payment_details['currency'] . "/" . $payment_details['type_of_payment'] . "/" . $payment_details['declear_value_carriage'] . "/" . $payment_details['declear_value_customs'] . "/" . $payment_details['declear_value_insurance'] . "<br>";
        //22
        $main_data .= "RTD/1/P" . $consignment_data['pieces'] . "/K" . $consignment_data['gross_weight'] . "/" . $consignment_data['rate_class'] . "/W" . $consignment_data['chargable_weight'] . "/R" . $consignment_data['rate'] . "/T" . $house_data['total_amount'] . "NC/" . $consignment_data['description'] . "<br>";
        //12 pieces information(dimsion) 
        $pieces_info = json_decode($consignment_data['pieces_info'], true);
        $main_data .= "/2/ND//" . ($pieces_info[0]['gross_weight'] ? 'K' . $pieces_info[0]['gross_weight'] . "/" : '') . $pieces_info[0]['unit'] . $pieces_info[0]['length'] . "-" . $pieces_info[0]['width'] . "-" . $pieces_info[0]['height'] . "/" . $pieces_info[0]['pcs'] . "<br>";
        ...
        if (!empty($house_data['other_service_information']))
            $main_data .= "OSI/" . substr($house_data['other_service_information'], 0, 65) . "<br>";
        if (!empty(substr($house_data['other_service_information'], 65, 65)))
            $main_data .= "/" . substr($house_data['other_service_information'], 65, 65) . "<br>";
        if (!empty(substr($house_data['other_service_information'], 130, 65)))
            $main_data .= "/" . substr($house_data['other_service_information'], 130, 65) . "<br>";
        //20
        $main_data .= "REF///" . substr($agent_details['prticipant_identifer'], 0, 3) . "/" . substr(str_replace(' ', '', $agent_details['participant_code']), 0, 17) . "/" . substr($agent_details['participant_airport'], 0, 3) . "<br>";
        //21
        $main_data .= "COR/" . substr($house_data['customs_origin_code'], 0, 2) . "<br>";
        //25
        $main_data .= "SPH";
        if ($house_data['special_handling_info']) {
        ...
        //30
        $main_data .= "HAH/" . $agent_details['ho_name'] . "<br>";
        $main_data .= "/" . $agent_details['ho_address'] . "<br>";
        $main_data .= $agent_details['ho_city'] . "/" . $agent_details['ho_pincode'] . "<br>";
        $main_data .= "/" . $agent_details['ho_state'] . "/" . $agent_details['ho_country'];
        echo $main_data;
    }

    // CONSOLIDATION
    public function ConsolidationConversion($awb_id = "12312345678")
    {
        $waybill_data = AirwayBills::select(['id', 'awb_no', 'awb_code','departure_airport','destination_airport'])->where('id', $awb_id)->first()->toArray();
        $house_data = HousewayBills::select(['id', 'awb_no', 'awb_code', 'departure_airport', 'destination_airport', 'special_handling_info'])->where('awb_no', $waybill_data['awb_no'])->where('awb_code', $waybill_data['awb_code'])->get()->toArray();
        $consignment_data = ConsignmentData::select(['gross_weight', 'weight_code', 'pieces'])->where([['awb_id', $awb_id]])->limit(1)->first()->toArray();
        $main_data = 'FHL/5<br>';
        $main_data .= 'MBI/' . $waybill_data['awb_code'] . '-' . $waybill_data['awb_no'] . substr($waybill_data['departure_airport'], 0, 3) . substr($waybill_data['destination_airport'], 0, 3) . '/T' . $consignment_data['pieces'] . 'K' . $consignment_data['gross_weight'] . "<br>";
        for ($i = 0; $i < sizeof($house_data); $i++) {
            $house_consignment_data = ConsignmentData::select(['gross_weight', 'weight_code', 'pieces', 'description'])->where([['awb_id', $house_data[$i]['id']]])->limit(1)->first()->toArray();
            $main_data .= 'HBS/' . $house_data[$i]['id'] . '/' . substr($house_data[$i]['departure_airport'], 0, 3) . substr($house_data[$i]['destination_airport'], 0, 3) . '/' . $house_consignment_data['pieces'] . '/K' . $house_consignment_data['gross_weight'] . '//' . substr($house_consignment_data['description'], 0, 15);
            $special_handling_info = json_decode($house_data[$i]['special_handling_info']);
            if (isset($special_handling_info[0]))
                $main_data .= '/' . $special_handling_info[0];
            $main_data .= "<br>";
        }
        echo $main_data;
    }
