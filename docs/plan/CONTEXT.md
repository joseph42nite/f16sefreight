...
2. ~~**`ui_ux_guide.md` has NOT had the reconciliation pass.**~~ ✅ **Done 2026-08-28 — `ui_ux_guide.md` §0a.** It was a different class of surprise from the other three: most of what it describes is simply unbuilt (10/13 routes, 10/13 components, 6 libraries), which is expected at this stage. The four real conflicts are the token system not existing at all, a font contradiction, a breakpoint name collision with Bootstrap 4, and — the trap — 🔴 **`resources/sass/` is not in the webpack build**, so `_variables.scss` silently swallows any change made to it.
