    ...
    addOrUpdateEntry: function addOrUpdateEntry(evt) {
      ...
      this.consignment_list.post("/get-consignment-error").then(function (response) {
        if (_this2.edit_entry_index !== null) {
          _this2.form.entries[_this2.edit_entry_index] = _objectSpread({}, _this2.consignment_list);
          _this2.edit_entry_index = null;
        } else {
          _this2.form.entries.push(_objectSpread({}, _this2.consignment_list));
        }
        _this2.closeModal();
        ...
        for (var key in _this2.consignment_list) {
          if (key != 'busy' && key != 'successful' && key != 'errors' && key != 'originalData') {
            if (_typeof(_this2.consignment_list[key]) === 'object') {
              _this2.consignment_list[key] = []; // Reset objects if needed
            } else {
