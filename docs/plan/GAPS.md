...
| # | Item | Detail |
|---|---|---|
| 50 | 🔴 **FocusSea has no inbox classification vocabulary.** `partners.partner_type` carries `shipping_line`, but the classification set — `customer_enquiry \| airline \| clearance \| trucking_road \| other` — is the AIR one. A sea carrier has nowhere to go, so `GlobalDomainDirectory` refuses to learn one. ⚠️ **Briefly added to the air list on 2026-09-03 and BACKED OUT the same day**, on the owner's correction: a sea carrier is FocusSea's counterparty, and putting Maersk in an air operator's folder list is noise on every screen that never handles sea. The guide already says rules are scoped by `transport_mode` because air and sea use different units, routing tokens and reference formats — the vocabulary has to be per mode for the same reason. Needs the sea set defined (`shipping_line`, and whatever else FocusSea's inbox actually sorts by) before a sea carrier's domain can be learned at all |
