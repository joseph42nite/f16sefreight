...
| 252 | ⚪ **Not built / not measured** | Answer quality and the 0.35 threshold — need `OPENROUTER_API_KEY` and real documents. A tour cannot wait for a control that appears after the previous step's click. "Connect to Support Agent" (PRD §5.10's second quick action) is not built. Screenshots on tickets are still not uploaded (#35) |
