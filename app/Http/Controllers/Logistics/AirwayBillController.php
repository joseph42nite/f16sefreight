...
class AirwayBillController extends Controller
{
    ...
    private function saveShipperAddress($awb_no, $awb_code, $shipper_address, $is_shipper_address_save)
    {
        ...
        $validator = Validator::make($shipper_address, $this->partyRules([
            ...
            'ship_post_code' => 'required|max:15',
    ...
    private function saveConsigneeAddress($awb_no, $awb_code, $consignee_address, $is_consignee_address_save)
    {
        ...
        $validator = Validator::make($consignee_address, $this->partyRules([
            ...
            'cons_post_code' => 'required|max:15',
            'cons_state' => 'required|string|max:35',
    ...
    private function storeSections(Request $request): array
    {
        ...
        if (!empty($request->shipper_address['ship_name']) && ($request->status === 'draft' || (!empty($request->shipper_address['ship_country']) && !empty($request->shipper_address['ship_city'])))) {
