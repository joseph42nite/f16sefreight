...
| 310 | 🟡 **Security audit: dependencies** | Patched: guzzle 7.14→7.15.5 (host-check bypass, high), league/commonmark 2.8→2.10 (XSS/DoS in help documents, high), maatwebsite/excel 3.1.70 and phpspreadsheet 1.30.6 (high). ❓ **Left open, need a decision:** Laravel 9 is end of life — the fixes for its CRLF-in-email-rule (high), signed-URL path confusion and file-validation advisories exist only in Laravel 10+/12+; dompdf 2.0.8 (6 medium/low, SVG file reads) needs 3.x, which may move the AWB PDF layout. npm: 64 advisories, mostly build tools (webpack, postcss) — frontend work; the runtime ones are axios, xlsx (via vue-excel-xlsx), vue-pdf/pdfjs-dist, vuetify |
