...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ({
    ...
    normalizedItems: function normalizedItems() {
      ...
      if (this.searchPerformed && this.data_items && this.data_items.airway_bill && this.data_items.airway_bill !== null) {
        return [{
          ...
          house_way_bills: this.data_items.house_way_bills.map(function (hwb) {
  ...
  methods: {
    ...
    getHouseWayBills: function getHouseWayBills(item) {
      ...
      var key = "".concat(item.awb_code, "-").concat(item.awb_no);
      if (!this.house_way_bills[key]) {
        _core_services_api_service__WEBPACK_IMPORTED_MODULE_2__["default"].get("/user/house-way-bills/".concat(item.awb_code, "/").concat(item.awb_no)).then(function (response) {
    ...
    scopedSlots: _vm._u([{
      ...
      fn: function fn(row) {
        return [_c("router-link", {
          ...
          nativeOn: {
            click: function click($event) {
              return _vm.getAirWayBill(row.item.id);
            }
          }
        }, [_vm._v("\n                                    " + _vm._s(row.item.awb_code) + " " + _vm._s(row.item.awb_no) + "\n                                    ")])];
      }
    }, {
