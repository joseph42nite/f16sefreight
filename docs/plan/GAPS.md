...
| 338 | 🔴→🟢 **First real mailbox: no enquiry could ever be created, and colleagues' mail filed as enquiries** | User, 2026-09-17, on joseph@f16sefreight.com (pricing, F16s Base): "not seeing anything, open workspace not working, no dropdown for the operations guy". Found: the Outlook connection and the 1-month import worked (83 messages, 51 conversations) — but (1) **code vs PRD §5.2.3 ("regex stages, the operator mints"):** an enquiry number was only created when someone *changed* the classification to customer enquiry; imported mail arrives already filed as one, so the change was a no-op, Claim created nothing, and with no enquiry the workspace had no work, the Kanban and Enquiries pages were empty, and no shipment could exist to carry the Operator dropdown. 🟢 Claim (and a pricing reply that claims) now creates the enquiry for a conversation filed as a customer enquiry; the workspace shows **Create enquiry** on one that has none; choosing customer enquiry again creates it too. (2) **Classifier:** 39 of 51 were colleagues at @f16sefreight.com or automated senders, all filed as customer enquiries by the fallback. 🟢 New step after branch rules: mail from the connected mailbox's own domain, or from an automated sender (no-reply, notifications, newsletters, alerts, mailer-daemon; notifications./news./mail. domains) is filed **Other**. Joseph's 51 unclaimed conversations were re-filed with it: 39 → Other, 10 remain customer enquiries (Descartes service desk, Lusha, IATA among them — re-file by hand). ⚠️ The Operator dropdown still appears only once a shipment (job) exists — Claim → enquiry → Confirm shipment (choose the operator) → job. Tests: RealMailboxFixesTest; full suite 751 passing |
