  ...
  methods: {
    ...
    addOrUpdateEntry: function addOrUpdateEntry(evt) {
      ...
      this.consignment_list.post("/get-house-consignment-error").then(function (response) {
        if (_this5.edit_entry_index !== null) {
          _this5.form.entries[_this5.edit_entry_index] = _objectSpread({}, _this5.consignment_list);
          // this.$set(this.form.entries, this.edit_entry_index, { ...this.consignment_list });
          _this5.edit_entry_index = null;
        } else {
          _this5.form.entries.push(_objectSpread({}, _this5.consignment_list));
        }
        _this5.calculateTotalVolume();
        _this5.calculateTotalAmount();
        _this5.closeModal();
        //clear consignment_list data
        for (var key in _this5.consignment_list) {
          if (key != 'busy' && key != 'successful' && key != 'errors' && key != 'originalData') {
            if (_typeof(_this5.consignment_list[key]) === 'object') {
              _this5.consignment_list[key] = [];
            } else {
              _this5.consignment_list[key] = '';
            }
          }
    ...
    calculateTotalVolume: function calculateTotalVolume() {
      ...
      var totalVolume = this.form.entries.reduce(function (total, entry) {
        return total + entry.itemss.reduce(function (entryTotal, item) {
          ...
          var selectedUnit = _this6.form.entries.dimention_unit;
          var finalVolume = 0;
          switch (selectedUnit) {
            case 'CMQ':
              // cm³
    ...
    addPcsInfo: function addPcsInfo() {
      ...
      Object.keys(rules).forEach(function (field) {
        ...
        var value = _this7.consignment_list[field];
        if (value) {
          if (rule.type === 'numeric' && (isNaN(value) || value < rule.min || value > rule.max)) {
            _this7.validationErrors.push(rule.message);
