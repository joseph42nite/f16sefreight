...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ({
  ...
  methods: {
    /** Keep the pagination total in sync with client-side filtering. */onFiltered: function onFiltered(filteredItems) {
     ...
     * @returns {Promise}
     */
    loadItems: function loadItems(url) {
      var _this = this;
      var transform = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
      this.items = [];
      this.isLoading = true;
      return _core_services_api_service__WEBPACK_IMPORTED_MODULE_0__["default"].get(url).then(function (_ref) {
        var data = _ref.data;
        _this.items = transform ? transform(data) : data;
     ...
     */
    confirmRemove: function confirmRemove(url) {
      var message = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "Are you sure you want to proceed?";
      if (!confirm(message)) return Promise.resolve(false);
      return _core_services_api_service__WEBPACK_IMPORTED_MODULE_0__["default"]["delete"](url).then(function () {
        return true;
      });
    }
  }
  ...
  methods: {
    ...
    getFilteredLocations: function getFilteredLocations(query) {
      ...
      var filtered = this.location.filter(function (item) {
        return item.iata_code.toLowerCase().includes(q) || item.destination && item.destination.toLowerCase().includes(q);
      });
      ...
      filtered.sort(function (a, b) {
        ...
        var aExactIata = aIata === q;
        var bExactIata = bIata === q;
        if (aExactIata && !bExactIata) return -1;
        if (!aExactIata && bExactIata) return 1;

        // 2. Starts with IATA code
        var aStartsIata = aIata.startsWith(q);
        var bStartsIata = bIata.startsWith(q);
        if (aStartsIata && !bStartsIata) return -1;
        if (!aStartsIata && bStartsIata) return 1;

        // 3. Starts with destination name
        var aStartsDest = aDest.startsWith(q);
        var bStartsDest = bDest.startsWith(q);
     ...
     */
    countryCodeByName: function countryCodeByName(name) {
      if (!name) return '';
      var target = String(name).toLowerCase();
      var match = (this.countries || []).find(function (c) {
        return c && c.text && c.text.toLowerCase() === target;
      });
      return match ? match.value : '';
    },
    calculateSimilarity: function calculateSimilarity(str1, str2) {
      if (!str1 || !str2) return 0;
      var s1 = this.normalizeText(str1);
      var s2 = this.normalizeText(str2);
      if (!s1 || !s2) return 0;
      if (s1 === s2) return 1.0;
    ...
    findMatchingAddress: function findMatchingAddress(ocrEntity, savedList) {
      ...
      for (var i = 0; i < savedList.length; i++) {
        ...
        var nameScore = this.calculateSimilarity(ocrEntity.name, savedItem.name);
        ...
        if (!savedItem.address || !ocrEntity.address || !ocrEntity.address.trim()) {
          ...
          var normSavedAddr = this.normalizeText(savedItem.address);
          var normOcrFull = this.normalizeText(ocrEntity.full_details || ocrEntity.address);
          if (normOcrFull && normSavedAddr && (normOcrFull.includes(normSavedAddr) || normSavedAddr.includes(normOcrFull))) {
            addressPass = true;
          } else {
            var addrScore = this.calculateSimilarity(savedItem.address, ocrEntity.address);
            if (addrScore >= 0.85) {
              addressPass = true;
            }
          }
...
function loadLocations() {
  ...
  inFlight = _core_services_api_service__WEBPACK_IMPORTED_MODULE_0__["default"].get("/user/get-location").then(function (_ref) {
    var data = _ref.data;
    cache = data;
    inFlight = null;
    return cache;
