...
| 272 | 🔴 **Fit to form saves suggested values** | `fit(party)` runs `cleanParty` over every part, including a state or country that was only *worked out* (low confidence), and writes the result back as a manual value with high confidence. That bypasses `withoutWorkedOutParts`, so a guess is saved as if someone typed it. Its real use is the House AWB's tight limits (address 40, state 9); now that every row has Edit, it may not be needed — **your call** |
