...
| 125 | 🔴 **`UserSeeder` has been unable to run since tenancy was added**, quite apart from idempotence. It never set `branch_name`, which is NOT NULL with no default | It now attaches to an **existing** branch rather than creating one — inventing a company here would duplicate `FreightDemoSeeder`, which owns demo tenants, and two seeders minting companies is how a database ends up with two half-configured ones. It warns and skips when no branch exists |
