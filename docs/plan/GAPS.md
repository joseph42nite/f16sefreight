...
| 277 | 🔴 **Doc vs code: the MAWB's address line 2 limit** | awbFieldRules.js said a master's line 2 takes 255; AirwayBillController allows **30**, and only `[a-zA-Z0-9\s.,-]` (notify: letters, digits, spaces) — not the widened address set of #44 that line 1 and the HAWB use. The rules file now says 30, since the server decides. The character set is left as it is: a "/" or "&" in a MAWB line 2 is still refused by the server. **Your call** whether the MAWB line 2 gets the widened set. Only reached when a MAWB address passes 255, or someone types line 2 |
| 278 | 🔴 **The paste box reads the town as part of the address** | Pasting "KALAMASEERY, KERALA 683503, INDIA" as the last line gave city KERALA and put KALAMASEERY at the end of the address (`parsePartyBlock`). Not changed here |
