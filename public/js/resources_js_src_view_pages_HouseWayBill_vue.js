  ...
  methods: {
    ...
    getAgent: function getAgent() {
      ...
      _core_services_api_service__WEBPACK_IMPORTED_MODULE_2__["default"].get("/agent-info/").then(function (_ref2) {
        ...
        if (Array.isArray(data) && data.length > 0) {
          _this3.agent_information = data[0];
        } else {
          _this3.agent_information = data;
        }
      })["catch"](function (error) {
    ...
    addOrUpdateEntry: function addOrUpdateEntry(evt) {
      ...
      this.consignment_list.post("/get-house-consignment-error").then(function (response) {
        if (_this4.edit_entry_index !== null) {
          _this4.form.entries[_this4.edit_entry_index] = _objectSpread({}, _this4.consignment_list);
          // this.$set(this.form.entries, this.edit_entry_index, { ...this.consignment_list });
          _this4.edit_entry_index = null;
        } else {
          _this4.form.entries.push(_objectSpread({}, _this4.consignment_list));
        }
        _this4.calculateTotalVolume();
        _this4.calculateTotalAmount();
        _this4.closeModal();
        //clear consignment_list data
        for (var key in _this4.consignment_list) {
          if (key != 'busy' && key != 'successful' && key != 'errors' && key != 'originalData') {
            if (_typeof(_this4.consignment_list[key]) === 'object') {
              _this4.consignment_list[key] = [];
            } else {
              _this4.consignment_list[key] = '';
            }
          }
    ...
    calculateTotalVolume: function calculateTotalVolume() {
      ...
      var totalVolume = this.form.entries.reduce(function (total, entry) {
        return total + entry.itemss.reduce(function (entryTotal, item) {
          ...
          var selectedUnit = _this5.form.entries.dimention_unit;
          var finalVolume = 0;
          switch (selectedUnit) {
            case 'CMQ':
              // cm³
    ...
    addPcsInfo: function addPcsInfo() {
      ...
      Object.keys(rules).forEach(function (field) {
        ...
        var value = _this6.consignment_list[field];
        if (value) {
          if (rule.type === 'numeric' && (isNaN(value) || value < rule.min || value > rule.max)) {
            _this6.validationErrors.push(rule.message);
