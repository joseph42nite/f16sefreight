...
5. **Model access:** Gemma 4 31B through OpenRouter (`python/model_extract.py`, `app/Services/Help/OpenRouterClient.php`): strict JSON-schema output, `provider.data_collection: deny`. **Cheap first, fast fallback always:** (1) named economy providers under a price ceiling (CoreWeave, Chutes, DeepInfra, Venice), (2) the named fast provider (ModelRun) then any, (3) economy by throughput — each try with a real total deadline (invoice 9/8/12 s, scan 15/15/20 s, help 6/6/8 s). Provider lists and limits are settings; measurements in GAPS #262–264. *(Was Ollama with `OLLAMA_KEEP_ALIVE` and a `Modelfile`, retired 2026-09-14.)*
...
| # | Item | State |
|---|---|---|
| 1 | `JobInbox.vue` | ✅ three columns, triage, claim, **split-pane collapse**. 🔴 **The workspace lost four of its six tabs on 2026-09-01, and two of them were lying.** *Enquiry* and *Timing* held two read-only fields and four timestamps — always true of the thread and unchanged while the drawer is open, so the tab charged a click for something that should simply be stated. They are in the drawer header now, with timing as a derived STATE (`Unanswered — 2d since triage` / `Answered 26m after triage`) rather than four datetimes: the value is the CONTRAST between triaged and replied, and a header full of raw stamps leaves the reader to do the subtraction. *Upload* duplicated `[Analyze PDF]`, which already uploads from the conversation header beside the attachment that needs reading — and its placeholder promised "Step 6 item 2", which **is** that modal, already built. *E-Docket*'s placeholder cited "item 4", which is `JobCostSheet` — built, and already the Cost sheet tab in the same drawer. ⚠️ **A placeholder naming a step already delivered is worse than none:** it tells an operator to wait for something they could be using. What remains is Extraction and Cost sheet — two actual work surfaces |
...
| 9 | `UpgradeTeaser.vue` | ✅ |
| 10 | Copilot · VisualReporter · tours | 🟢 **Help copilot built 2026-09-14** (GAPS #246–252): `HelpAssistant.vue` in the header, answers from help documents F16s uploads per page (superadmin → Help documents), vectors in MySQL, Gemma 4 31B on OpenRouter, guided steps with Driver.js on `data-help` controls, hand-off to `VisualReporter.vue` with the conversation attached. 🟡 **Screenshot upload still blocked on GAPS #35**; "Connect to Support Agent" not built |
...
| File | Asserts |
|---|---|
| `JobTriageTest.php` ✅ | `ENQA-`/`ENQS-` sequence assignment; observer seeds the initial `Intake` milestone. The prefix comes from the **mode**, never the portal — a cross-mode caller (import, console, accounts) has no portal at all. Also asserts re-entering a milestone writes a **second row**: a timestamp column per milestone would overwrite the first visit and hide the rework, which is the number anybody investigating a slow branch wants |
...
| `EnquirySequenceConcurrencyTest.php` ✅ | Parallel processes generate non-overlapping numbers; all generation routes through `EnquirySequenceService`. 🔴 **"Parallel" is literal — spawn real OS processes.** A sequential loop passes against an implementation with NO locking at all, which is how a guaranteed deadlock survived every earlier test. This file caught it: six of eight minters died with `SQLSTATE 40001` (GAPS.md). Children run `tests/Support/mint_sequence.php`, **not** `artisan tinker` — PsySH aborts on a piped stdout — and must be handed the test database explicitly. Assert **contiguity**, not just uniqueness: a lost update shows up as a hole |
| `ConcurrencyClaimingTest.php` ✅ | Simultaneous claims — the loser receives `409`. Six processes race the **real endpoint** through `tests/Support/http_request.php`, which boots the kernel in-process; racing a hand-written copy of the controller's query would keep passing after the controller stopped doing it |
| `CrossTenantIsolationTest.php` | Branch A cannot read or edit branch B (`403`/`404`); party references share the acting `company_id` |
| `MultiPortalScopingTest.php` ✅ | Background jobs correctly bypass portal scope — and the pass-through is proved to be the **absence of a binding** rather than a scope that never filtered, or the test passes against a gutted `forActivePortal()`. Also: a cross-mode portal (`accounts`) still cannot see another **tenant** — portal scope is not tenant scope; and an unknown host is refused **404, not 403**, naming no real portal |
...
| `InvoiceFinalizeTest.php` ✅ | Balanced double-entry posting and the GST split are asserted in `CreditGateTest`/`GstSplitTest`; this file covers the two clauses nothing else reached. 🔴 **A period that exists and is `closed` is a different path from no period at all** — an implementation checking `whereNotNull` rather than `status = 'open'` passes the older test and still posts into a filed month. ⚠️ **`403` vs `422` is unresolved** (GAPS.md #37): the guide and PRD say 403, the code returns 422, so the test asserts only that it is refused and that **no ledger row is written**. The concurrency clause found a live defect — finalized invoices kept their `DRAFT-…` placeholder, so assert the number **starts `INV-`**, and **force a cold start** by deleting the counter row, or every child takes the deadlock-free UPDATE path |
...
| `PdfOcrTierBranchingTest.php` ⚠️ | Core + a structured AWB → `/extract`, no credit; **Core + an invoice or scan → `failure_code = 'upgrade_required'` with no FastAPI call at all**; Tactical + a text PDF → Gemma, an `llm_usage_logs` row, **no credit and no prompt**; **Tactical + a scan → `awaiting_vision_consent` with the balance UNCHANGED and no Gemini call**; declining → `cancelled`, still nothing spent; accepting → exactly 1 credit and a `consumption` row carrying `pdf_processing_job_id`; accepting with `balance <= limit` → `credits_exhausted` **before** any FastAPI call; a Gemini timeout after reserving → refund + `ai_unavailable`; **a retried job cannot refund twice — the second attempt violates `UNIQUE (reverses_transaction_id)`**; an unanswered prompt expires at 24 h, cancels, and deletes the temp PDF |
| `ReassignmentFlowTest.php` ✅ | Request pins with elevated priority; **withdraw hard-deletes the notification**; accept promotes pending → live. ⚠️ All three are already asserted in `BellNotificationTest`; this file covers the ways a handover is left **stranded** instead, and found four (GAPS.md) — a direct reassignment leaving a bell card and a staging timestamp behind, a request on an unowned job that notified nobody, and a second request stacking a second pinned card. None of them look wrong from the acting user's side |

> 🟢 **The consent path was BUILT on 2026-09-01, not merely tested.** Writing this file's
> consent clauses turned up that nothing wired `OcrRoutingService`/`OcrCreditService` to a
> human: no accept/decline endpoint, no route, no UI. A scanned PDF parked and was swept
> 24h later, so vision OCR could never run and `reserve()` had no reachable caller — see
> GAPS.md. `VisionConsentTest` (15 tests) now covers accept, decline, expiry, the
> ownership gate, double-answer, and the refund lookup.
>
> ⚠️ Still uncovered here, and blocked on **gap #29**: the Tactical text-PDF → Gemma path
...
| ~~**"Cost to book" as a queue**~~ | ✅ **Built 2026-09-26** — Money out ① lists every billed shipment with no cost booked, oldest billed first, and accounts books the cost inline: supplier (the AWB's airline by default), charge, amount, GST % typed not defaulted. Booked through the cost sheet's own endpoint, so one path raises a voucher. After billing the sheet opens for ADDING a cost only, accounts only; the sell side and pricing stay locked, and nothing is ever added to a posted voucher (GAPS #398) |
