    ...
    public function downloadConsolidationPdf($awb_code, $awb_no)
    {
        $airWayBill = AirWayBills::where('awb_code', $awb_code)
            ...
            if($airWayBill)
            {
                ...
                $pdf = Pdf::loadView('documents.generate-consolidation-pdf', compact('airWayBill', 'houseWayBills', 'agentInfo', 'wayBillAddress', 'wayBillConsignmentData', 'paymentInfo'))->setPaper('a4', 'portrait')->set_option('isHtml5ParserEnabled', true);
                return $pdf->stream();
            }
