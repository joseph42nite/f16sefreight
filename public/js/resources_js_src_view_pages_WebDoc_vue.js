...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ({
  ...
  methods: {
    generateAwbPDF: function generateAwbPDF() {
      ...
      var pdfUrl = "/download-awb-pdf/".concat(itemId); // Construct the URL for the PDF
    ...
    addOrUpdateEntry: function addOrUpdateEntry(evt) {
      ...
      this.consignment_list.post("/get-consignment-error").then(function (response) {
        if (_this12.edit_entry_index !== null) {
          _this12.form.entries[_this12.edit_entry_index] = _objectSpread({}, _this12.consignment_list);
          // this.$set(this.form.entries, this.edit_entry_index, { ...this.consignment_list });
          _this12.edit_entry_index = null;
        } else {
          _this12.form.entries.push(_objectSpread({}, _this12.consignment_list));
        }
        _this12.calculateTotalVolume();
        _this12.calculateTotalAmount();
        _this12.closeModal();
        //clear consignment_list data
        for (var key in _this12.consignment_list) {
          if (key != 'busy' && key != 'successful' && key != 'errors' && key != 'originalData') {
            if (_typeof(_this12.consignment_list[key]) === 'object') {
              _this12.consignment_list[key] = [];
            } else {
              _this12.consignment_list[key] = '';
            }
          }
    ...
    calculateTotalVolume: function calculateTotalVolume() {
      ...
      var totalVolume = this.form.entries.reduce(function (total, entry) {
        return total + entry.itemss.reduce(function (entryTotal, item) {
          ...
          var selectedUnit = _this13.form.entries.dimention_unit;
          var finalVolume = 0;
          switch (selectedUnit) {
            case 'CMQ':
              // cm³
    ...
    addPcsInfo: function addPcsInfo() {
      ...
      Object.keys(rules).forEach(function (field) {
        ...
        var value = _this14.consignment_list[field];
        if (value) {
          if (rule.type === 'numeric' && (isNaN(value) || value < rule.min || value > rule.max)) {
            _this14.validationErrors.push(rule.message);
      ...
      if (awb_code && awb_code.length === 3) {
        _core_services_api_service__WEBPACK_IMPORTED_MODULE_2__["default"].get("/get-awbcode-prefix/".concat(awb_code)).then(function (response) {
          if (response.data) {
            ...
            _this15.awb_prefix_message = "Message will be sent to ".concat(name, " (").concat(code, ")");
          } else {
            _this15.awb_prefix_message = "No details found for this AWB code.";
          }
        })["catch"](function (error) {
          console.error("Error fetching AWB details:", error);
          _this15.awb_prefix_message = "Failed to fetch AWB details.";
        });
      } else {
    ...
    reloadPageWithContent: function reloadPageWithContent() {
      ...
      _core_services_api_service__WEBPACK_IMPORTED_MODULE_2__["default"].get("/airway-bill/".concat(awbId)).then(function (response) {
        _this16.existingData = response.data;
        if (_this16.existingData) {
          _this16.awbDetails = false;
          _this16.openForm('update', _this16.existingData.id);
          _this16.$router.push({
            path: "/edit-airway-bill/".concat(awbId)
          });
          _this16.location.reload();
          // this.$router.push({ path: `/edit-airway-bill/${awbId}` });
        } else {
          _this16.awbDetails = false;
        }
      })["catch"](function (error) {
        _this16.existingData = null;
  ...
  mounted: function mounted() {
    ...
    var _this$$route$query = this.$route.query,
      awb_code = _this$$route$query.awb_code,
      awb_no = _this$$route$query.awb_no;
    if (awb_code && awb_no) {
      this.form.first_box.awb_code = awb_code;
      this.form.first_box.awb_no = awb_no;
    }
    var awbId = this.$route.params.awbId;
